...
assert.doesNotMatch(html, /PERSONAL MEANS PERSONAL/);
assert.doesNotMatch(html, /SIGNAL, WITH YOUR PERMISSION/);
assert.doesNotMatch(html, /WEEKLY ACTIVITY/);
assert.doesNotMatch(html, /WHO TO TALK TO/);
assert.doesNotMatch(html, /HUSH LINE · TIP INBOX/);
assert.doesNotMatch(html, /class="eyebrow"/);
assert.match(html, /A product of Hush Line, a 501\(c\)\(3\) non-profit in the United States\./);
const footer = html.match(/<footer\b[^>]*>([\s\S]*?)<\/footer>/)?.[1] || '';
assert.match(footer, /A product of Hush Line/);
assert.doesNotMatch(footer, /The missing relationship layer for your Mac/);
